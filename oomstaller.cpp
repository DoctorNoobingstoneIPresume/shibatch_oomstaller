...
std::unordered_map<int, ProcInfo> getProcesses() {
  ...
  while((entry = readdir(procdir))) {
    ...
    if (fstat(fileno(fp), &statbuf) == 0 && statbuf.st_uid == uid) {
      try {
	ProcInfo pi(fp, entry->d_name, pageSize);
	ret[pi.pid] = pi;
...
void showUsage(const std::string_view& argv0, const std::string_view& mes = std::string_view{}) {
  if (! mes.empty()) std::cerr << mes << "\n\n";
  std::cerr << "\n";
  std::cerr << "NAME\n";
  std::cerr << "     oomstaller - suppress swap thrashing at build time\n";
  std::cerr << "\n";
  std::cerr << "SYNOPSIS\n";
  std::cerr << "     " << argv0 << " [<options>] command [arg] ...\n";
  std::cerr << "\n";
  std::cerr << "DESCRIPTION\n";
  std::cerr << "     This tool monitors the memory usage of each process when performing a\n";
  std::cerr << "     build, and suspends processes as necessary to prevent swap thrashing\n";
  std::cerr << "     from occurring.\n";
  std::cerr << "\n";
  std::cerr << "  --max-parallel <number of processes>            default:   0\n";
  std::cerr << "\n";
  std::cerr << "     Suspends processes so that the number of running build processes\n";
  std::cerr << "     does not exceed the specified number. 0 means no limit. A process is\n";
  std::cerr << "     counted as one process even if it has multiple threads.\n";
  std::cerr << "\n";
  std::cerr << "  --max-parallel-thrash <number of processes>     default:   1\n";
  std::cerr << "\n";
  std::cerr << "     Specifies the maximum number of processes to run when thrashing is\n";
  std::cerr << "     detected. 0 means no limit.\n";
  std::cerr << "\n";
  std::cerr << "  --period <seconds>                              default:   1.0\n";
  std::cerr << "\n";
  std::cerr << "     Specifies the interval at which memory usage of each process is checked\n";
  std::cerr << "     and processes are controlled.\n";
  std::cerr << "\n";
  std::cerr << "  --show-stat\n";
  std::cerr << "\n";
  std::cerr << "     Displays statistics when finished.\n";
  std::cerr << "\n";
  std::cerr << "TIPS\n";
  std::cerr << "     If you kill this tool with SIGKILL, a large number of build processes\n";
  std::cerr << "     will remain suspended with SIGSTOP. To prevent this from happening,\n";
  std::cerr << "     use SIGTERM or SIGINT to kill this tool. You can send SIGCONT to all\n";
  std::cerr << "     processes run by you with the following command.\n";
  std::cerr << "\n";
  std::cerr << "     killall -v -s CONT -u $USER -r '.*'\n";
  std::cerr << "\n";
  std::cerr << "AUTHOR\n";
  std::cerr << "     Written by Naoki Shibata.\n";
  std::cerr << "\n";
  std::cerr << "     See https://github.com/shibatch/oomstaller\n";
  std::cerr << "\n";
  std::cerr << "oomstaller 0.4.0\n";
  std::cerr << "\n";
  std::cerr.flush ();
